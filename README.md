...
以下検討した改善点です！
```
サーバー起動時に歌手とアルバムテーブルを新規作成しているので、SQLなどを使用してサーバーの再起動でデータがすべて初期化されるのを防ぐ必要があると思いました。

アルバムのアーティストが複数になる可能性を考えて、1対多の関係から中間テーブルを用いた多対多の関係にしたほうが今後の拡張性が高くなると思いました。
```